...
//#include "ds_monitoring.h" /* Assignment8 */
//DECLARE_DS_MONITORING(pxt4_write_dm);
...
static int __init pxt4_init_fs(void)
{
	...
	printk("[%s] current->cpu=%d, smp_pro=%d\n", __func__, current->cpu, smp_processor_id());
...
static void __exit pxt4_exit_fs(void)
{
	...
	// Assignment8
	//print_ds_monitoring(&pxt4_write_dm);
	//delete_ds_monitoring(&pxt4_write_dm);
}
