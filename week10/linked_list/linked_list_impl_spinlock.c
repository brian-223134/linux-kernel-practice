...
void search_list(int thread_id, int range_bound[])
{
    ...
    // [최적화] 리스트를 한 번만 순회하며 내 범위의 노드를 확인합니다. (O(N))
    spin_lock(&my_spinlock);
...
{
    int i;
    pr_info("spinlock_module_init: Entering Spinlock Module!\n");
...
static void __exit spinlock_module_exit(void)
{
    ...
    pr_info("spinlock_module_cleanup: Spinlock linked list insert time: ");
    ktprint(0, insert_time);
    pr_info("spinlock_module_cleanup: Spinlock linked list search time: ");
    ktprint(0, search_time);
    pr_info("spinlock_module_cleanup: Spinlock linked list delete time: ");
    ktprint(0, delete_time);
    pr_info("spinlock_module_cleanup: Exiting Spinlock Module!\n");
}
...
MODULE_DESCRIPTION("Linked List Synchronization with Spinlock");
