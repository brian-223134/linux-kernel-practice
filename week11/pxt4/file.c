...
#include "xattr.h"
#include "acl.h"
#include "calclock.h" //clock test for Assignment7 -> same file name but different (Assignment9)
#include <linux/ktime.h>

//#include "mm/filemap.h" // Assignment11

extern ssize_t __pxt4_generic_file_write_iter(struct kiocb *iocb, struct iov_iter *from);
...
static unsigned long get_cpu_idx(void *elem)
{
    return current->cpu;
}
...
{
    static char buf[16];
    scnprintf(buf, sizeof(buf), "CPU%u", current->cpu);
    return buf;
}

static void print_cpu_dm(unsigned long cpu_id,
                         const char *name,
                         unsigned long long count,
                         int percentage)
{
    printk(KERN_INFO "[pxt4 dm] %s (id=%lu): %llu calls (%d%%)\n",
           name ? name : "CPU", cpu_id, count, percentage);
}

//DEFINE_DS_MONITORING(pxt4_write_dm, get_cpu_idx, get_cpu_name, print_cpu_dm);
...
//pxt4_file_write_iter -> pxt4_file_write_iter_internal
//for Assingnment
static ssize_t
...
//	.write_iter	= pxt4_file_write_iter_internal,
